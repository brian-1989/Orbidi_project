...
from app.endpoints.description import create_location_category_reviewed_description as description
...
def create_location_category_reviewed(review_schema: ReviewedSchema, db: Session = Depends(get_db)):
    return JSONResponse(
        jsonable_encoder(create_location_category_reviewed_in_db(reviewed_schema=review_schema, db=db)),
